...
(function ($) {
	...
	VLTJS.animatedBlock = {
		init: function () {
			var el = $('.bd-animated-block');
			el.each(function () {
				var $this = $(this);
				VLTJS.window.on('bd.change-slide', function () {
					$this.removeClass('animated');
					if ($this.parents('.bd-section').hasClass('active')) {
...
(function ($) {
	...
	VLTJS.counterUp = {
		init: function () {
			...
			var el = $('.bd-counter-up, .bd-counter-up-small');
...
(function ($) {
	...
	VLTJS.customCursor = {
		init: function () {
			if (!$('.bd-is--custom-cursor').length) {
				return;
			}
			VLTJS.body.append('<div class="bd-custom-cursor"><div class="circle"><span></span></div></div>');
			var cursor = $('.bd-custom-cursor'),
			...
			if (typeof gsap != 'undefined') {
				...
				VLTJS.document.on('mouseenter', 'input, textarea, select, .bd-video-button > a', function () {
					gsap.to(circle, .3, {
						scale: 0,
						opacity: 0
					});
				}).on('mouseleave', 'input, textarea, select, .bd-video-button > a', function () {
					gsap.to(circle, .3, {
						scale: 1,
						opacity: .1
					});
...
(function ($) {
	...
	VLTJS.fullpageSlider = {
		init: function () {
			...
			var el = $('.bd-fullpage-slider'),
				progress_bar = el.find('.bd-fullpage-slider-progress-bar'),
				numbers = el.find('.bd-fullpage-slider-numbers'),
			...
			function vlthemes_navbar_solid() {
				if (el.find('.pp-section.active').scrollTop() > 0) {
					$('.bd-navbar').addClass('bd-navbar--solid');
				} else {
					$('.bd-navbar').removeClass('bd-navbar--solid');
				}
			}
			...
			function vlthemes_page_brightness() {
				var section = el.find('.bd-section.active');
			...
			el.pagepiling({
				menu: '.bd-offcanvas-menu ul.sf-menu, .bd-fullpage-slider-progress-bar',
				scrollingSpeed: speed,
				loopTop: loop_top,
				loopBottom: loop_bottom,
				anchors: anchors,
				sectionSelector: '.bd-section',
				navigation: false,
				afterRender: function () {
					vlthemes_show_navigation();
					vlthemes_page_brightness();
					vlthemes_slide_counter();
					VLTJS.window.trigger('bd.change-slide');
				},
...
(function ($) {
	...
	VLTJS.menuOffcanvas = {
		...
		init: function () {
			var menu = $('.bd-offcanvas-menu'),
				navigation = menu.find('ul.sf-menu'),
				navigationItem = navigation.find('> li'),
				header = $('.bd-offcanvas-menu__header'),
				footer = $('.bd-offcanvas-menu__footer > div'),
				menuOpen = $('.js-offcanvas-menu-open'),
				menuClose = $('.js-offcanvas-menu-close'),
				siteOverlay = $('.bd-site-overlay');
...
(function ($) {
	...
	el.on('animsition.inEnd', function () {
		VLTJS.window.trigger('bd.preloader_done');
		VLTJS.html.addClass('bd-is-page-loaded');
	});
})(jQuery);
/***********************************************
 * WIDGET: PROGRESS BAR
			...
			el.each(function () {
				...
				VLTJS.window.on('bd.change-slide', function () {
					if ($this.parents('.bd-section').hasClass('active')) {
						...
						obj.count = 0;
						$this.find('.bd-progress-bar__title > .counter').text(Math.round(obj.count));
						gsap.set($this.find('.bd-progress-bar__bar > span'), {
							width: 0
						});
						...
						gsap.to($this.find('.bd-progress-bar__bar > span'), animation_duration / 1000, {
							width: final_value + '%',
							delay: delay / 1000
						});
...
(function ($) {
	...
	VLTJS.projectShowcase = {
		initSlider: function () {
			...
			var el = $('.bd-project-showcase-slider .swiper-container');
			new Swiper(el, {
				speed: 1000,
				spaceBetween: 30,
				grabCursor: true,
		...
		initParallax: function () {
			...
			var el = $('.bd-project-showcase'),
				items = el.find('.bd-project-showcase__items'),
				item = items.find('.bd-project-showcase__item'),
				images = el.find('.bd-project-showcase__images'),
				image = images.find('.bd-project-showcase__image'),
				wDiff,
				value;
...
(function ($) {
	...
	VLTJS.testimonialSlider = {
		init: function () {
			...
			var el = $('.bd-testimonial-slider .swiper-container');
			el.each(function () {
				var $this = $(this);
				...
				new Swiper(this, {
					...
					slidesPerView: 1,
					navigation: {
						nextEl: $('.bd-testimonial-slider-controls .next'),
						prevEl: $('.bd-testimonial-slider-controls .prev'),
					},
					pagination: {
						el: $('.bd-testimonial-slider-controls .pagination'),
						clickable: false,
						type: 'fraction',
...
(function ($) {
	...
	VLTJS.timelineSlider = {
		init: function () {
			...
			var el = $('.bd-timeline-slider .swiper-container');
			el.each(function () {
				var $this = $(this);
				$this.find('.swiper-wrapper > *').wrap('<div class="swiper-slide">');
				new Swiper(this, {
					...
					slidesPerView: 1,
					navigation: {
						nextEl: $('.bd-timeline-slider-controls .next'),
						prevEl: $('.bd-timeline-slider-controls .prev'),
					},
					pagination: {
						el: $('.bd-timeline-slider-controls .pagination'),
						clickable: false,
						type: 'fraction',
						renderFraction: function (currentClass, totalClass) {
							return '<span class="' + currentClass + '"></span>' +
